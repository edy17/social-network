...
public class PostRepository extends AbstractBaseRepository<Post> {
    ...
    public PutItemRequest putRequest(Post post) {
        ...
        Stream.of(Post.class.getDeclaredFields()).forEach(field -> {
            try {
                ...
                try {
                    if (item.containsKey(field.getName())) {
                        ...
                        if (field.getName().equals("image")) {
                            ...
                            List<Comment> comments = mapper.readValue(json, mapper.getTypeFactory().constructCollectionType(List.class, Comment.class));
